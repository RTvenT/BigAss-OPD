...
class HUD:
    def __init__(self, player=None):
        ...
        self.weapon_slots_y = WINDOW_HEIGHT - 100  # Размещаем внизу экрана
        self.weapon_names = {
            'Pistol': 'Пистолет',
            'Shotgun': 'Дробовик',
            'AutoRifle': 'Автомат'
    ...
    def draw_weapon_slots(self, surface):
        ...
        # Фон для всех слотов
        slots_bg_width = (self.weapon_slot_size * 3) + (self.weapon_slot_gap * 2) + 20
        slots_bg_height = self.weapon_slot_size + 40
        slots_bg_rect = pygame.Rect(
            center_x - slots_bg_width // 2,
            self.weapon_slots_y - 10,
            slots_bg_width,
            slots_bg_height
        )
        pygame.draw.rect(surface, (20, 20, 20), slots_bg_rect)
        pygame.draw.rect(surface, (40, 40, 40), slots_bg_rect, 2)
        ...
        for i in range(3):
            ...
            if i < len(self.player.weapons):
                ...
                if hasattr(weapon, 'weapon_surf'):
                    weapon_image = pygame.transform.scale(weapon.weapon_surf, (48, 48))
    ...
    def draw_boss_timer(self, surface):
        if self.time_until_boss > 0:
            ...
            boss_surface = self.boss_font.render(boss_text, True, (255, 50, 50))

            # Размещаем в центре верхней части экрана
            boss_x = (surface.get_width() - boss_surface.get_width()) // 2
            boss_y = 60
            ...
            pygame.draw.rect(surface, (0, 0, 0, 128), boss_bg)
            pygame.draw.rect(surface, (255, 0, 0), boss_bg, 2)
